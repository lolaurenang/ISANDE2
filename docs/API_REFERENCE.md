...
| Method | Path | Access | Purpose |
|---|---|---|---|
| GET | `/availability` | any | Your days. `?from=` `?to=` (managers may pass `?employee=`) |
| GET | `/availability/roster` | manager | Who is free each day. `?view=` `?date=` |
| POST | `/availability` | any | Save one day (upsert) |
| POST | `/availability/bulk` | any | Save many: `{ "dates": ["2026-08-05", …] }` |
| DELETE | `/availability/:workDate` | any | Take a day back |
